...
This repo publishes the `remote-mcp` plugin to the [official MCP Registry](https://registry.modelcontextprotocol.io) automatically on git tag.

## Cut a release

1. Bump `version` in:
   - [`plugins/remote-mcp/server.json`](../plugins/remote-mcp/server.json)
   - [`plugins/remote-mcp/.cursor-plugin/plugin.json`](../plugins/remote-mcp/.cursor-plugin/plugin.json)
